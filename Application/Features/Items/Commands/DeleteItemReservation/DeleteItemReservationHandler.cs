...
namespace Application.Features.Items.Commands.DeleteItemReservation
{
    public class DeleteItemReservationHandler : IRequestHandler<DeleteItemReservationCommand, Result<List<ModelResponseDto>>>
    {
        ...
        public async Task<Result<List<ModelResponseDto>>> Handle(DeleteItemReservationCommand request, CancellationToken cancellationToken)
        {
            ...
            if (item.CheckedOutBy == 0) return Result<List<ModelResponseDto>>.Failure(new Error("O item não está reservado"));
            if (item.CheckedOutBy != request.UserId) return Result<List<ModelResponseDto>>.Failure(new Error("O item não está reservado para este usuário"));
            ...
            if (!itemCheckedOut) return Result<List<ModelResponseDto>>.Failure(new Error("Ocorreu um erro ao tentar excluir a reserva do item o item"));
            ...
            {
                return Result<List<ModelResponseDto>>.Failure(new Error("Ocorreu um erro ao tentar excluir os arquivos  da workspace"));
            }
